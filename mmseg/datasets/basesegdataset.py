...
class BaseSegDataset(BaseDataset):
    ...
    #     return data_list
    def load_data_list(self) -> List[dict]:
        """Load the image/annotation pairs from a JSON-Lines split file.

        ``data_prefix['img_path']`` points at a ``{train,val,test}.json`` file
        whose lines are JSON objects with ``image_path`` and ``seg_mask_path``
        keys.  The file is produced by ``tools/prepare_dataset.py``.

        Paths inside the file may be absolute, or relative to ``data_root``
        (the behaviour of ``tools/prepare_dataset.py``), which keeps a prepared
        dataset movable between machines.

        Returns:
            list[dict]: All data info of dataset.
        """
        ann_file = self.data_prefix.get('img_path', None)
        data_list = []
        with open(ann_file, 'rt') as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                ...
                seg_map_path = item['seg_mask_path']
