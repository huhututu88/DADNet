...
@TRANSFORMS.register_module()
class RandomRotate_Choices(BaseTransform):
    """Rotate the image & seg.
    ...
    def __init__(self,
        ...
        self.center = center
        self.auto_bound = auto_bound
        assert isinstance(degree, list) or isinstance(degree, tuple)
...
@TRANSFORMS.register_module()
class Random_Choices(BaseTransform):
    def __init__(self,
                 operations=None,
                 pad_val=0,
                 seg_pad_val=255):
        ...
        if operations is None:
            self.operations = ['RandomFlip']
        else:
            self.operations = operations
        self.pal_val = pad_val
        self.seg_pad_val = seg_pad_val
        assert isinstance(self.operations, list) or isinstance(self.operations, tuple)

    def transform(self, results: dict) -> dict:
        selected = random.choice(self.operations)
        if selected == 'RandomRotate_Choices':
            results = RandomRotate_Choices(degree=[
                    90,
                    180,
                    270,
                ])(results)
            # print('已经执行RandomRotate_Choices')
        elif selected == 'RandomFlip':
            if random.random() > 0.5:
                results = RandomFlip(direction='horizontal', prob=1.0)(results)
                # print('已经执行horizontal')
            if random.random() < 0.5:
                results = RandomFlip(direction='vertical', prob=1.0)(results)
                # print('已经执行vertical')
        else:
            pass
        return results
